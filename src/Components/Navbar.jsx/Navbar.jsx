  ...
  return (
    <nav className="flex flex-wrap justify-between md:items-center text-white px-10 pt-6 md:px-20">
      ...
      <ul
        className={` 
          ${menu ? "block" : "hidden"}
          mx-18 py-2 mt-4 font-semibold md:mt-5 bg-black/30 px-10 rounded-xl bg-opacity-30 md:border-none text-center 
      ...
      {showMenu ? (
        ...
        <RiCloseLine
          size={30}
          className="md:hidden absolute right-10 top-6 transition-all duration-300"
          onClick={handleToggle} // Yeh line important hai!
        />
      )}
    </nav>
  );
